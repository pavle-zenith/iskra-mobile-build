...
;(function () {
// HomeScreenDay0.jsx — Iskra home EMPTY STATE (Dan 0). First screen after onboarding.
// Based on HomeScreenV3 — only the empty-state values differ.
...
function GoalsSection({ onOpen = () => {}, onOpenRoadmap = () => {}, onSlip = () => {} }) {
  const teal = '#2E8B80';
  const GOALS = [
    { key: 'zdravlje', name: 'Zdravlje', color: '#D4547E', pct: 0, Icon: ActivityIcon3 },
    { key: 'pluca', name: 'Pluća', color: '#4A8AC4', pct: 0, Icon: LungsIcon3 },
    { key: 'ekologija', name: 'Ekologija', color: '#3A7A3A', pct: 0, Icon: LeafIcon3 },
    { key: 'finansije', name: 'Finansije', color: '#2E8B80', pct: 0, Icon: CoinIcon3 },
    { key: 'telo', name: 'Telo', color: '#C4724A', pct: 0, Icon: UserIcon3 },
    { key: 'nikotin', name: 'Nikotin', color: '#6B52A8', pct: 0, Icon: MoleculeIcon3 },
  ];
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 13, marginTop: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
        {GOALS.map((g) => (
          <button key={g.key} onClick={() => onOpen(g.key)} style={{
            ...
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#CCC" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ position: 'absolute', top: 8, right: 8 }}>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 20, fontWeight: 500, color: ISK3.text, letterSpacing: '-0.01em', whiteSpace: 'nowrap' }}>0 dana 0 sati</div>
...
function WeekTracker3({ onCheckIn = () => {}, done = false }) {
  const days = [
    { l: 'Pon', s: 'future' }, { l: 'Uto', s: 'future' }, { l: 'Sre', s: 'future' },
    { l: 'Čet', s: 'future' }, { l: 'Pet', s: 'future' }, { l: 'Sub', s: 'today' },
    { l: 'Ned', s: 'future' },
  ];
  return (
    <button onClick={onCheckIn} style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <span style={{ fontSize: 12.5, fontWeight: 600, color: '#CCC', whiteSpace: 'nowrap' }}>0 / 7</span>
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        {days.map((d) => (
          <div key={d.l} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 9 }}>
            ...
            {d.s === 'future' && (
              <div style={{ width: 30, height: 30, borderRadius: '50%', background: '#fff', border: `1px solid #E8E8E8` }} />
            )}
          </div>
        ))}
      </div>
...
function Countdown3({ onClick }) {
  ...
  const cells = [
    { v: 0, l: 'DANA' }, { v: '00', l: 'SATI' },
    { v: '00', l: 'MINUTA' }, { v: '00', l: 'SEKUNDE' },
  ];
  return (
    <button onClick={onClick} style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 13 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 700, color: ISK3.brand, letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>POČINJE DANAS</span>
      </div>
      ...
      <div style={{ fontSize: 12, fontWeight: 400, color: '#BBB', textAlign: 'center', marginTop: 13 }}>Tajmer počinje kad zatvoriš app</div>
    </button>
  );
}

function StatCard3({ icon, value, label, sub, cat, onClick }) {
  const C = CAT[cat];
  return (
    <button onClick={onClick} style={{
      ...
      {sub && <div style={{ fontSize: 11, color: '#BBB', marginTop: 2, fontWeight: 500 }}>{sub}</div>}
    </button>
  );
}
...
function HomeScreenDay0() {
  ...
  if (view === 'porivMode' && window.PorivMode) return <window.PorivMode level={porivLevel} onClose={() => setView('home')} />;
  return (
    <div style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '58px 20px 10px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 14, fontWeight: 700, color: ISK3.brandDeep }}>1</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <StatCard3 cat="novac" onClick={() => window.MoneyScreen && setView('money')} icon={(c) => <CoinIcon3 size={20} stroke={c} sw={2} />} value="0" label="RSD uštedeno" sub="Počinje da raste" />
            <StatCard3 cat="cigare" onClick={() => window.CigarettesScreen && setView('cigarettes')} icon={(c) => <CigOffIcon3 size={20} stroke={c} sw={1.9} />} value="0" label="cigareta odbijeno" sub="Sve pred tobom" />
          </div>
          ...
          {/* Moji ciljevi */}
          <GoalsSection onOpen={(key) => { if (window.CategoryScreen && key === 'zdravlje') setView('cat:' + key); }} onOpenRoadmap={() => { if (window.GoalsRoadmapScreen) setView('roadmap'); }} onSlip={() => { if (window.SlipScreen) setView('slip'); }} />
        </div>
      </div>
      ...
      <div style={{ flexShrink: 0, padding: '8px 20px 0' }}>
        ...
        <div style={{
          ...
          <NavItem3 icon={(c) => <FlagIcon3 size={23} stroke={c} sw={tab === 'ms' ? 2.1 : 1.9} />} label="Milestoni" active={tab === 'ms'} onClick={() => setTab('ms')} />
...
Object.assign(window, { HomeScreenDay0 });
