...
function GoalsSection({ onOpen = () => {}, onOpenRoadmap = () => {} }) {
  const teal = '#2E8B80';
  const GOALS = [
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        {days.map((d) => (
          <div key={d.l} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 9 }}>
            <span style={{ fontSize: 11.5, fontWeight: 600, color: (d.s === 'today' || d.s === 'celebrate') ? '#9B8FA8' : ISK3.sub }}>{d.l}</span>
            {d.s === 'done' && (
              <div style={{
            ...
            {d.s === 'today' && (
              <div style={{
                width: 30, height: 30, borderRadius: '50%',
                background: '#9B8FA8', border: 'none',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                boxShadow: '0 2px 5px rgba(155,143,168,0.3)',
              }}>
...
function Countdown3({ onClick }) {
  const quitRef = React.useRef(Date.now());
  ...
  return (
    <button onClick={onClick} style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 13 }}>
        <SparkIcon3 size={14} stroke={'#9B8FA8'} sw={2} />
        <span style={{ fontSize: 12, fontWeight: 700, color: '#9B8FA8', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>NOV POČETAK</span>
      </div>
...
function HomeScreenSlip() {
  ...
  return (
    <div style={{
      ...
      {checkIn && <CheckInOverlay onClose={() => setCheckIn(false)} onClean={() => { setCheckIn(false); setWeekDone(true); }} onSlip={() => setCheckIn(false)} />}
      <div style={{ flex: 1, overflowY: 'auto', padding: '58px 20px 10px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
          ...
          <div style={{
            ...
            background: '#F0EDF8', borderRadius: 999, padding: '8px 13px',
          }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#9B8FA8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M12 2v20M4.5 7l15 10M19.5 7l-15 10" /><path d="M12 6l3-2M12 6 9 4M12 18l3 2M12 18l-3 2M4.5 9.5l.3-3.5M19.5 9.5l-.3-3.5M4.5 14.5l.3 3.5M19.5 14.5l-.3 3.5" />
            </svg>
            <span style={{ fontSize: 14, fontWeight: 700, color: '#9B8FA8' }}>1</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <StatCard3 cat="cigare" onClick={() => window.CigarettesScreen && setView('cigarettes')} icon={(c) => <CigOffIcon3 size={20} stroke={c} sw={1.9} />} value="939" label="cigareta odbijeno" />
          </div>
          ...
          {/* Moji ciljevi */}
          <GoalsSection onOpen={(key) => { if (window.CategoryScreen && key === 'zdravlje') setView('cat:' + key); }} onOpenRoadmap={() => { if (window.GoalsRoadmapScreen) setView('roadmap'); }} />
        </div>
      </div>
...
Object.assign(window, { HomeScreenSlip });
